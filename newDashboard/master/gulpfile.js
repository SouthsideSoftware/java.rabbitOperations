...
log("useSourceMaps=" + useSourceMaps)
...
gulp.task('styles:app', function () {
  ...
  return gulp.src(source.styles.app)
    ...
    .on('error', handleError)
    .pipe($.if(isProduction, $.minifyCss()))
    .pipe($.if(useSourceMaps, $.sourcemaps.write('./maps')))
...
gulp.task('nosourcemaps', ['nosources', 'default']);
gulp.task('nosources', function () {
  useSourceMaps = false;
});
...
gulp.task('cleanAssets', function () {
  log('Deleting compiled css, js and html');
  del([build.styles, paths.app + '/views', paths.app + '/pages'], {force: true});
});
